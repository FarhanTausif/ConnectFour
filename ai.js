...
function aiMove() {
    ...
    for (let c = 0; c < columns; c++) {
        if (currColumns[c] >= 0) {
            ...
            currColumns[c]--;
            if (checkWinner(true) === playerBrown) {
                board[r][c] = ' ';
                currColumns[c]++;
                let tile = document.getElementById(r.toString() + "-" + c.toString());
                board[r][c] = playerBrown;
...
//     // Log initial state for this recursive call
//     console.log(`Minimax (Depth: ${depth}, ${isMaximizing ? 'Maximizing (AI)' : 'Minimizing (Human)'}) - Initial: alpha=${alpha}, beta=${beta}`);

//     let result = checkWinner(true);
//     if (result !== null) {
//         let score = result === playerBrown ? 1000 : result === playerRed ? -1000 : 0;
//         console.log(`Terminal state reached (Depth: ${depth}) - Result: ${result === 'draw' ? 'Draw' : result === playerBrown ? 'AI Win' : 'Human Win'}, Score: ${score}`);
//         return score;
//     }
//     if (depth === 0) {
//         let score = evaluateBoard();
//         console.log(`Depth limit reached (Depth: ${depth}) - Evaluated board score: ${score}`);
//         return score;
//     }
//     if (isMaximizing) {
...
//                 currColumns[c]--;
//                 // Log the move
//                 console.log(`Move (AI): Placed piece at Row ${r}, Column ${c} (Depth: ${depth}) - Before: alpha=${alpha}, beta=${beta}`);
//                 let eval = minimax(depth - 1, alpha, beta, false);
//                 board[r][c] = ' ';
//                 currColumns[c]++;
//                 // Log the outcome
//                 console.log(`Move (AI): Row ${r}, Column ${c} (Depth: ${depth}) - After: eval=${eval}, updated alpha=${Math.max(alpha, eval)}, beta=${beta}`);
//                 maxEval = Math.max(maxEval, eval);
//                 alpha = Math.max(alpha, eval);
//                 if (beta <= alpha) {
//                     console.log(`Pruning after Move (AI): Row ${r}, Column ${c} (Depth: ${depth}) - beta=${beta} <= alpha=${alpha}`);
//                     break;
//                 }
//             }
//         }
...
//                 currColumns[c]--;
//                 // Log the move
//                 console.log(`Move (Human): Placed piece at Row ${r}, Column ${c} (Depth: ${depth}) - Before: alpha=${alpha}, beta=${beta}`);
//                 let eval = minimax(depth - 1, alpha, beta, true);
//                 board[r][c] = ' ';
//                 currColumns[c]++;
//                 // Log the outcome
//                 console.log(`Move (Human): Row ${r}, Column ${c} (Depth: ${depth}) - After: eval=${eval}, alpha=${alpha}, updated beta=${Math.min(beta, eval)}`);
//                 minEval = Math.min(minEval, eval);
//                 beta = Math.min(beta, eval);
//                 if (beta <= alpha) {
//                     console.log(`Pruning after Move (Human): Row ${r}, Column ${c} (Depth: ${depth}) - beta=${beta} <= alpha=${alpha}`);
//                     break;
//                 }
//             }
//         }
...
// === Win/Draw Detection ===
function checkWinner(isForMinimax = false) {
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < columns - 3; c++) {
            if (board[r][c] != ' ') {
                if (board[r][c] == board[r][c+1] && board[r][c+1] == board[r][c+2] && board[r][c+2] == board[r][c+3]) {
                    if (isForMinimax) return board[r][c];
                    setWinner(r, c);
                    return;
                }
            }
    ...
    if (currColumns.every(col => col < 0)) {
        if (isForMinimax) return 'draw';
        setWinner(0, 0);
        return;
    }
    if (isForMinimax) return null;
}
